...
int main(int argv, char** argc) {
  ...
  */
  ...
  for (int i = 1; i < argv; ++i) { 
    std::cout << "FILE : " << argc[i] << " contains... " << std::endl;    
    std::string str;
    std::ifstream myfile (argc[i]);
  ...
  labels = {"Total"};//{"PPFXMIPPKaon","PPFXMIPPPion","PPFXOther","PPFXTargAtten","PPFXThinKaon","PPFXThinMeson","PPFXThinNeutron","PPFXThinNucA","PPFXThinNuc","PPFXThinPion","PPFXTotAbsorp","Total"};
  ...
  for (gallery::Event ev(filename) ; !ev.atEnd(); ev.next()) {
	...
	//      std::cout << "BNB Correction Weight : " << cv_weight << std::endl;
	...
	for(int l = 0; l < int(labels.size())-1; l++){
	  for(unsigned int i = 0; i < Weights[l].size(); i++){
	    ...
	    Enu_Syst_Window[pdg][l][i]->Fill(mctruth.GetNeutrino().Nu().E());
	    
	    if(intercept){	  
	      Enu_Syst_AV_TPC[pdg][l][i]->Fill(mctruth.GetNeutrino().Nu().E());
	    }
	  }
	}
	...
	for(unsigned int i = 0; i < Weights[0].size(); i++){
	  Enu_Syst_Window[pdg][full_lable][i]->Fill(mctruth.GetNeutrino().Nu().E());
	  
	  if(intercept){	  
	    Enu_Syst_AV_TPC[pdg][full_lable][i]->Fill(mctruth.GetNeutrino().Nu().E());
	  }	
	}
      }
