...
void CreateFriendTree(const char * filename, const char * treename) {
   ...
   strcat(filename2, "friend.root");
   TFile *ff = new TFile(filename2,"recreate");
   TTree *TF = T->CopyTree("TrackMomMCS_mom>0.45 && TrackMomMCS_mom<0.77");
   TF->SetName(treename);
   //TF->BuildIndex("sec");
   TF->Write();
   TF->Print();
...
void groupby_point_V4(const char* filename, int event_size, int is_data_ = 1, int with_genie = 0){
  //usage: filename, bignumber, data = 1, ext = 2, MC =0, withgenie=-1 (data,ext) 1 or 10
  FILE *data = 0;
  FILE *text = 0; 
  int has_wire = 0;
  ...
  if(with_genie == 1 ||with_genie == 10) t_in->SetBranchAddress("All_Genie", &All_Genie);
  ...
  if(with_genie == 1) strcat(name,"out4wG.root");
  else strcat(name,"out4.root");
  ...
  */
  t_out->Branch("crtt0_time",        &crtt0_time_out,              "crtt0_time/D");
  t_out->Branch("crtt0_trig",        &crtt0_trig_out,              "crtt0_trig/I");
  t_out->Branch("crtt0_DCA",         &crtt0_DCA_out,              "crtt0_DCA/D");
  t_out->Branch("crtt0_plane",       &crtt0_plane_out,              "crtt0_plane/I");
  /*
  ...
  for(long i = 0; i< tot_ev; i++){
    ...
    for(unsigned int j = 0; j<track_key_->size(); j++){
      ...
      if(track_key_->at(j) == key_muon){
        ...
        if(!is_data_){
          MCTrackPurity_out = MCTrackPurity_->at(j);
          MCTrackPDG_out = MCTrackPDG_->at(j);
          MCTrackEnergy_out = MCTrackEnergy_->at(j);
          MCTrackMomentum_out = MCTrackMomentum_->at(j);
          MCTrackTheta_out = MCTrackTheta_->at(j);
          MCTrackPhi_out = MCTrackPhi_->at(j);
          MCTrackLength_out = MCTrackLength_->at(j);
          MCTrackStart_x_out = MCTrackStart_x_->at(j);
          MCTrackStart_y_out = MCTrackStart_y_->at(j);
          MCTrackStart_z_out = MCTrackStart_z_->at(j);
          MCTrackEnd_x_out = MCTrackEnd_x_->at(j);
          MCTrackEnd_y_out = MCTrackEnd_y_->at(j);
          MCTrackEnd_z_out = MCTrackEnd_z_->at(j);
        }
    ...
    for(unsigned int j = 0; j<crthit_ts0_->size(); j++){
      ...
      if(is_data_!=2){
        if(crthit_ts0_->at(j)<4.95 && crthit_ts0_->at(j)>2.85){
      ...
      if(is_data_==2){ //this means ext with a missing time correction
        if(crthit_ts0_->at(j)<(4.95+3.57-3.195) && crthit_ts0_->at(j)>(2.85+3.57-3.195)){
          //printf("Found CRT hit in beam!\n");
          nr_crthit_beam_++;
          if(crt_adc_->at(j)>70){
            nr_crthit_beam_tres_++;
    ...
    crthit_x_out = -9999, crthit_y_out = -9999, crthit_z_out = 9999, crthit_plane_out = -1;
    a_crthit_x_out = -9999, a_crthit_y_out = -9999, a_crthit_z_out = -9999, a_crthit_plane_out = -1;
    ...
    MCTrackPurity_out = 0, MCTrackPDG_out = 0, MCTrackEnergy_out = 0;
    MCTrackMomentum_out = 0, MCTrackTheta_out = 0, MCTrackPhi_out = 0;
    MCTrackLength_out = 0;
    MCTrackStart_x_out = 0, MCTrackStart_y_out = 0, MCTrackStart_z_out = 0;
    MCTrackEnd_x_out = 0, MCTrackEnd_y_out = 0, MCTrackEnd_z_out = 0;
    //MCle_Vx_out = -9999;
    //MCle_Vy_out = -9999;
    //MCle_Vz_out = -9999;
