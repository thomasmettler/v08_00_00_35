...
private:
  ...
  //lar_pandora::HitVector wirevector;
  ...
  float MCle_Vx_sce, MCle_Vy_sce, MCle_Vz_sce;
  */
  TTree* _sr_tree;
  int _sr_run = -9999;
  int _sr_subrun = -9999;
...
void NumuCCana::analyze(art::Event const & evt)
{
  ...
  if(store_Spacepoints){
    ...
    for (auto const hit : hitvector){
      ...
      //auto space_cor = spacepoint->XYZ();
      //auto space_cor = space_cor1.XYZ();
      if(this_wire<100 && this_plane==2){
        Wire_id_.push_back(this_wire );
        Wire_plane_.push_back(this_plane );
      }
      //SpacePoint_y_.push_back(space_cor[1]);
      //SpacePoint_z_.push_back(space_cor[2]);
      //std::cout << "Found Hit in channel:" << this_wire << " on plane: " << this_plane << std::endl;
      //spacepoint->XYZ()[0]
    }
  }
  ...
  if(verbose_!=0) std::cout << "[NumuCCana::filter] Get metadata and build particvle map" << std::endl;
  ...
  if (pfparticles.size() == 0){ //this should never happen
    ...
    if(verbose_!=0) std::cout << "[NumuCCana::filter] Get neutrino candidate" << std::endl;
    ...
    if (pfneutrinos.size() != 1){ // some events (e.g out of TPC) have no neutrino reconstructed
      ...
      if(verbose_!=0) std::cout << "[NumuCCana::filter] Get vertex" << std::endl;
    ...
    if (!pfp->IsPrimary()){ 
      ...
      if (particlesToTracks.find(pfp) != particlesToTracks.end() ){ // get the track like pfp
        ...
        if(fill_per_track_==1){
          if(verbose_!=0) std::cout << "[NumuCCana::getVertex] get Muon: " << NuScore_ << std::endl;
          if (!GetMuon(pfp, MCSMu_handle, trackPIDAssn)){ // check if muon candidate fullfills all the requirements
            std::cout << "[NumuCCana::getVertex] Event failed: Daughter investigation unsuccessful" << std::endl;
            //return false;
          }
          else if (MatchDaughter(evt, pfp)){
            std::cout << "[NumuCCana::getVertex] Matched MC daughter successfully" << std::endl;
            //fill tree
            my_event_->Fill();
          }
          else std::cout << "[NumuCCana::getVertex] Matched MC daughter not successfull" << std::endl;
        }
        else if (MatchDaughter(evt, pfp)){
          std::cout << "[NumuCCana::getVertex] Matched MC daughter successfully" << std::endl;
        }
...
bool NumuCCana::GetMuon(const art::Ptr<recob::PFParticle> &pfp,
  ...
  //if(verbose_!=0) std::cout << "[NumuCCana::GetMuon] Got track score: " << TrackScore_ << std::endl;
  ...
  if (particlesToTracks.find(pfp) != particlesToTracks.end()){ // always true
    ...
    TrackDir_x_ = track_dir.X();
    TrackDir_y_ = track_dir.Y();
    TrackDir_z_ = track_dir.Z();
    TrackStart_x_ = this_track->Start().X();
    TrackStart_y_ = this_track->Start().Y();
    TrackStart_z_ = this_track->Start().Z();
    TrackEnd_x_ = this_track->End().X();
    TrackEnd_y_ = this_track->End().Y();
    TrackEnd_z_ = this_track->End().Z();
    TrackTheta_ = this_track->Theta();
    TrackPhi_ = this_track->Phi();*/
    
    //const recob::MCSFitResult &mcsMu = MCSMu_handle->at(this_track.key());
    //TrackMomMCS_mom_ = mcsMu.fwdMomentum();
    //TrackMomMCS_err_ = mcsMu.fwdMomUncertainty();
    //TrackMomMCS_ll_ = mcsMu.fwdLogLikelihood();
    
    //trackHelper.getRangeMomentum(TrackLength_, TrackMomRange_p_, TrackMomRange_mu_);
...
bool NumuCCana::MatchDaughter(art::Event const &evt, const art::Ptr<recob::PFParticle> &pfp)
{
  ...
  if (fGeneration == 2)
  {
    if (matchedParticles.find(pfp) == matchedParticles.end()){
      std::cout << "[NuCC::MatchDaughter] no match end of particles for gen2. SIze: " << matchedParticles.size() << std::endl;
      return false;
    }
  ...
  else if (fGeneration == 3)
  {
    ...
    if (matchedParticles.find(pfp_parent) == matchedParticles.end()){
      std::cout << "[NuCC::MatchDaughter] no match end of particles. SIze: " << matchedParticles.size() << std::endl;
      return false;
    }
  ...
  if (!is_data_)
  {
    ...
    if(verbose_!=0) std::cout << "[NumuCCana::MatchDaughter] " << "entered function" << std::endl;
    lar_pandora::MCTruthToMCParticles truthToParticles;
    lar_pandora::MCParticlesToMCTruth particlesToTruth;

    lar_pandora::LArPandoraHelper::CollectMCParticles(evt, m_geant_producer, truthToParticles, particlesToTruth);

    for (auto iter : particlesToTruth)
    {
      if (iter.first->TrackId() == matched_mcp->TrackId())
      {
        if(verbose_!=0) std::cout << "[NumuCCana::MatchDaughter] " << "give return" << std::endl;
        mctruth = iter.second;
        //return iter.second;
      }
    }
  ...
  if (m_hasMCNeutrino)
  {
    ...
    MCle_pfp_.push_back(pfp->Self());
    MCle_key_.push_back(pfp.key());
    MCle_PDG_.push_back(matched_mcp->PdgCode());
    MCle_purity_.push_back(matchedHitFraction);
    MCle_Energy_.push_back(matched_mcp->E());
    MCle_Px_.push_back(matched_mcp->Px());
    MCle_Py_.push_back(matched_mcp->Py());
    MCle_Pz_.push_back(matched_mcp->Pz());
    MCle_Vx_.push_back(matched_mcp->Vx());
    MCle_Vy_.push_back(matched_mcp->Vy());
    MCle_Vz_.push_back(matched_mcp->Vz());
    MCle_length_.push_back((matched_mcp->Position().Vect() - matched_mcp->EndPosition().Vect()).Mag());
    /*
    MCle_PDG = matched_mcp->PdgCode();
    MCle_purity = matchedHitFraction;
    MCle_Energy = matched_mcp->E();
    MCle_Px = matched_mcp->Px();
    MCle_Py = matched_mcp->Py();
    MCle_Pz = matched_mcp->Pz();
    ...
    */
    ...
    auto MCle_Vtx_offset = SCE->GetCalPosOffsets(geo::Point_t(matched_mcp->Vx(), matched_mcp->Vy(), matched_mcp->Vz()));
    MCle_Vx_sce_.push_back(matched_mcp->Vx() - MCle_Vtx_offset.X());
    MCle_Vy_sce_.push_back(matched_mcp->Vy() + MCle_Vtx_offset.Y());
    MCle_Vz_sce_.push_back(matched_mcp->Vz() + MCle_Vtx_offset.Z());
...
void NumuCCana::FillTrueNuDaughters(art::Event const &evt)
{
  ...
  for (auto const &mcparticle : mcparticles)
  {
    ...
    art::Handle<std::vector<simb::MCTruth>> generator_handle;
    evt.getByLabel("generator", generator_handle); //what is the product instance name? no BernZMQ    
    //auto const &generator_handle = evt.getValidHandle<std::vector<simb::MCTruth>>("generator");
    if(!generator_handle.isValid()){
      std::cout << "[NumuCCana::FillTrueNuDaughters] invalid raw handle" << std::endl;
      return;
    }
    else if(verbose_!=0) std::cout << "[NumuCCana::FillTrueNuDaughters] raw handle is valid" << std::endl;
    //::art::ServiceHandle<cheat::ParticleInventoryService> pi;
    //const art::Ptr<simb::MCTruth> mc_truth = pi->TrackIdToMCTruth_P(mcparticle->TrackId());
    ///////////////////////////////////////////////////////////////////////////////////////////////
    art::Ptr<simb::MCTruth> mc_truth;
    ...
    for (auto iter : particlesToTruth)
    {
      if (iter.first->TrackId() == mcparticle->TrackId())
      {
        if(verbose_!=0) std::cout << "[NumuCCana::FillTrueNuDaughters] " << "give return" << std::endl;
        mc_truth = iter.second;
        //return iter.second;
      }
    }
    /////////////////////////////////////////////////////////////////////////////////////////////////
...
void NumuCCana::FillReconTruthMatching(art::Event const &evt)
{
  ...
  //int check = 0;
  std::cout << "Datalabels: " << m_pfp_producer << " - " << m_pfp_producer << " - " << m_hitfinder_producer << " - " << m_geant_producer << " - " << m_hit_mcp_producer << std::endl;
  pandoraInterfaceHelper.Configure(evt, m_pfp_producer, m_pfp_producer, m_hitfinder_producer, m_geant_producer, m_hit_mcp_producer);
  //if(verbose_!=0) std::cout << "[NumuCCana::FillReconTruthMatching] Configured: " << check << std::endl;
...
void NumuCCana::FillTrueNu(art::Event const &evt)
{
  ...
  if (!is_data_)
  {
    if(verbose_!=0) std::cout << "[NumuCCana::FillTrueNu] get handle" << std::endl;
    
    art::Handle<std::vector<simb::MCTruth>> generator_handle;
    evt.getByLabel("generator", generator_handle); //what is the product instance name? no BernZMQ    
    //auto const &generator_handle = evt.getValidHandle<std::vector<simb::MCTruth>>("generator");
    if(!generator_handle.isValid()){
      std::cout << "[NumuCCana::FillTrueNu] invalid raw handle" << std::endl;
      return;
    }
    //::art::ServiceHandle<cheat::ParticleInventoryService> pi;
    //const art::Ptr<simb::MCTruth> mc_truth = pi->TrackIdToMCTruth_P(mcparticle->TrackId());
    ...
    //if (mc_truth.size() > 0)
    {
      ...
      //const simb::MCNeutrino &mcnu = mc_truth.GetNeutrino();
      ...
      auto MCNu_Vtx_offset = SCE->GetCalPosOffsets(geo::Point_t(mcnu.Nu().Vx(), mcnu.Nu().Vy(), mcnu.Nu().Vz()));
      MCNu_Vx_sce = mcnu.Nu().Vx() - MCNu_Vtx_offset.X();
      MCNu_Vy_sce = mcnu.Nu().Vy() + MCNu_Vtx_offset.Y();
      MCNu_Vz_sce = mcnu.Nu().Vz() + MCNu_Vtx_offset.Z();
...
void NumuCCana::clearEvent(){ // reset all pfp related vectors
  ...
  MCNu_Vx_sce = -999, MCNu_Vy_sce = -999, MCNu_Vz_sce = -999;
...
void NumuCCana::initialize_tree()
{
  ...
  my_event_->Branch("ShowerScore",        &ShowerScore_);
  ...
  my_event_->Branch("isShowerTrack",       &isShowerTrack_);
  ...
  if(!is_data_){
    ...
    //my_event_->Branch("MCNu_leptonPhi",      &MCNu_leptonPhi,      "MCNu_leptonPhi/F");
    ...
    my_event_->Branch("MCle_Vx_sce",             &MCle_Vx_sce_);
    my_event_->Branch("MCle_Vy_sce",             &MCle_Vy_sce_);
    my_event_->Branch("MCle_Vz_sce",             &MCle_Vz_sce_);
